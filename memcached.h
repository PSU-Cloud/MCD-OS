...
#define NUM_SUB_LRU_LISTS 4   /* hot, warm, cold, temp */


//XXX: MCD-OS config

// make sure to set the number of thread workers to the
//  number of clients in the systems
#define NUM_WORKERS 9

// you can set the allocations per lru in thread.c:memcached_thread_init()
int allocation[NUM_WORKERS];
...
typedef struct {
    ...
    // struct _stritem *heads[NUM_SUB_LRU_LISTS];             /* heads of 4 sub-lru-lists */
    // struct _stritem *tails[NUM_SUB_LRU_LISTS];             /* tails of 4 sub-lru-lists */
    int id;                     /* number id of this thread */
    int alloc_size;             /* total size */
    int avail_size;       /* available size */
    int attrib_size;       /* allocated size */
#ifdef EXTSTORE
...
typedef struct _stritem {
    ...
    struct _stritem *next;        /* used in slab-LRU */
    struct _stritem *prev;
    struct _stritem *nexts[NUM_WORKERS];     /* used in proxy LRU */
    struct _stritem *prevs[NUM_WORKERS];
    /* Rest are protected by an item lock */
    struct _stritem *h_next;    /* hash chain next */
    rel_time_t      time;       /* least recent access */
    rel_time_t      exptime;    /* expire time */
    int             nbytes;     /* size of data */
    int             virtual_len;  /* shared length of data */
    unsigned short  refcount;
    uint16_t        it_flags;   /* ITEM_* above */
    uint16_t        it_flag[NUM_WORKERS];   /* status of item in each LRU */
    uint8_t         slabs_clsid;/* which slab class stores the value */
    uint8_t         lru[NUM_WORKERS];  /* which lru contains this item in each worker */
    uint8_t         nkey;       /* key length, w/terminating null and padding */ 
    /* this odd type prevents type-punning issues when we do
     * the little shuffle to save space when not using CAS. */
    //LIBEVENT_THREAD *thread;     /* which thread we're in */
    union {
        uint64_t cas;
        char end;
    } data[];
...
int   item_link(item *it, LIBEVENT_THREAD *thread);
void  item_remove(item *it);
int   item_replace(item *it, item *new_it, LIBEVENT_THREAD *thread, const uint32_t hv);
void  item_unlink(item *it, LIBEVENT_THREAD *thread);
